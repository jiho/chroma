...
test_that("reverse works", {
  expect_that(
    make_scale(reverse=TRUE)(c(0,1)),
    equals( rev(make_scale(reverse=FALSE)(c(0,1))) )
  )
})

test_that("data outside domain returns extreme color", {
  cols <- c("#000000", "#ffffff")
  bw <- make_scale(cols, domain=c(0,1))
...
test_that("values overrides domain", {
  cols <- c("#000000", "#ffffff")
  bw <- make_scale(cols, domain=c(0,1), values=c(3,4))
...
test_that("unsorted values returns error", {
  expect_error(make_scale(c("red", "green", "blue"), values=c(1,3,2)))
})

test_that("different number of values and colors returns error", {
  expect_error(make_scale(c("red", "green"), values=c(1,2,3)))
  expect_error(make_scale(c("red", "green", "blue"), values=c(1,2)))
})
